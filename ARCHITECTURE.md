...
현재는 로컬 목업으로 실행하며 Vercel과 Supabase에는 연결하지 않는다. 향후 Web과 stateless FastAPI는 Vercel, 영속 상태는 Supabase에 배치한다. collector는 국가법령정보 Open API에 등록된 고정 공인 IPv4 Windows PC의 OS 스케줄러에서 실행하고 Supabase로 검증 결과를 보낸다. 집 PC는 공개 인바운드 요청을 받지 않는다. API는 Python 3.14 런타임, 웹은 Node 24/pnpm 11을 사용한다. Supabase DB 연결 시에는 Supavisor transaction pooler를 전제로 prepared statement cache를 끈다.
