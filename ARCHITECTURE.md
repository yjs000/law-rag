...
```text
고정 공인 IP 서버의 collector ── JSON 우선/XML 폴백 ──> 국가법령정보 Open API
       │                         │
       │                         └─ HTML·PDF·외부 법률 사이트 금지
       └─ OS 스케줄러 ──> 정규화/해시/버전 ──> 목업 저장소(현재) / Supabase(후속)
                                                  │ PGroonga + pgvector
                                                  v
Browser ──> 단일 클라우드 서버
              ├─ Next.js web
              ├─ FastAPI API ──> RRF 검색 ──> OpenAI ports
              └─ collector(OS 스케줄러)       Responses + Structured Outputs
```

현재는 로컬 목업으로 실행하며 Vercel과 Supabase에는 연결하지 않는다. 향후 웹·API·collector는 같은 고정 공인 IP 클라우드 서버에 배치하되 독립 프로세스로 운영한다. API는 Python 3.14 런타임, 웹은 Node 24/pnpm 11을 사용한다. Supabase DB 연결 시에는 Supavisor transaction pooler를 전제로 prepared statement cache를 끈다.

Python 실행 단위는 같은 저장소 안에서 두 프로젝트로 분리한다.

- `apps/api`: 질문·검색·목업 인증·내보내기 API를 제공하는 장기 실행 웹 프로세스
- `apps/collector`: 국가법령정보 수집 전용 CLI/배치 프로젝트. API 프로세스와 독립적으로 같은 서버의 OS 스케줄러가 실행
...
- 키는 저장소에 커밋하지 않고 클라우드 서버의 비밀 저장소 또는 배포 환경변수에 둔다.
- 질문 원문, IP, 원문 전문을 로그에 남기지 않는다.
- AI 장애와 검색 장애를 분리한다.
- 법제처에 등록한 고정 공인 IP 서버에서 `apps/collector`를 별도 프로세스로 실행한다. 현재는 PC의 OS 스케줄러와 수동 명령을 사용하고, 클라우드 이전 시 웹·API와 같은 서버의 고정 출구 IP를 등록한다. 공용 runner와 브라우저에서 법령 API를 직접 호출하지 않는다.
- Supabase와 Vercel은 현재 목업 경계 뒤에 두며 실제 자격정보·프로젝트 연결은 후속 단계로 미룬다.
- 익명 질문은 저장하지 않는다. 목업 로그인 사용자의 질문 이력만 목업 저장소에 보존한다. 실제 로그인은 Google만 지원하며 질문 이력은 생성일부터 1년 보존 후 삭제한다. 계정 삭제 시 질문·이력·세션·내보내기 등 해당 사용자와 연결된 모든 데이터를 삭제한다.
