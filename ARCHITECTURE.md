...
| 질문 사전 라우팅 | 단일 NVIDIA 질문 라우터와 검색을 시작하지 않는 `routing_unavailable` 안전 경로 | [단일 단계 라우터와 라우터 불가 응답](docs/design-docs/single-stage-router-and-failure-response.md) · [0028 역사 기록](docs/design-docs/pre-retrieval-question-routing.md) · [0057 실행 계획](docs/exec-plans/completed/0057-single-stage-router-and-failure-response.md) |
...
| 2026-08-04 | [대체됨] v1 운영·실험 D를 포함한 제품·실험 경로에서 HNSW를 영구 제외하고 exhaustive exact cosine을 유지 | 당시 근사 인덱스 도입으로 품질 판정과 운영 복잡도를 다시 늘리지 않기로 한 사용자 결정. v1·실험 D에는 계속 적용되나, 별도 v2 LlamaIndex 테이블의 운영자 전용 예외는 2026-08-18 결정으로 추가됨 |
