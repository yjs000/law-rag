...
## Corpus Check
- 575 files · ~482,438 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 7694 nodes · 15221 edges · 495 communities (415 shown, 80 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 1260 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `cd3edbb2`
...
Cohesion: 0.10
Nodes (30): publish_prepared_bundle(), Path, Fail before maintenance unless the bundle closes every prospective vector gap., Validate, gate, drain and atomically publish a prepared corpus update., _require_complete_prospective_embeddings(), _bundle(), _Connection, asyncio (+22 more)
...
Cohesion: 0.09
Nodes (46): RawResponse, plan_provision_sync(), ProvisionRecord, raw_object_path(), _corpus_gate_call_indices(), _deletion_repository(), _DeletionConnection, _DeletionEngine (+38 more)

### Community 6 - "build_v1_answer_dependencies"
Cohesion: 0.09
Nodes (29): build_v1_answer_dependencies(), Application seams supplied by the entry point for v1 answer execution., Assemble the v1 use case without exposing its adapter implementations., V1ApplicationCallbacks, anonymous_rate_limit_subject(), _canonical_ip(), daily_subject_hash(), date (+21 more)
...
Cohesion: 0.10
Nodes (47): _arguments(), _article_contexts(), _article_root(), _atomic_publish_run(), _atomic_write_query_cache(), _cache_file_sha256(), _cache_key(), _canonical_json_bytes() (+39 more)
...
Cohesion: 0.08
Nodes (68): embedding_text_sha256(), EmbeddingProfile, legal_provision_embedding_text(), Build the versioned passage text used for provision embeddings., load_provisions(), load_provisions_from_connection(), AsyncConnection, Current-parser corpus records used by Experiment D validation and retrieval. (+60 more)
...
Cohesion: 0.09
Nodes (49): test_content_snapshot_identity_does_not_include_the_calendar_date(), BundleState, _atomic_write(), _build_manifest(), canonical_corpus_population_fingerprint(), canonical_corpus_publish_snapshot_id(), canonical_corpus_snapshot_id(), _canonical_json() (+41 more)

### Community 13 - "MemoryLegalRepository"
Cohesion: 0.07
Nodes (43): _date_or_none(), MemoryLegalRepository, date, Path, UUID, test_quota_resets_on_next_day(), MonkeyPatch, test_anonymous_question_search_failure_returns_safe_temporary_error() (+35 more)
...
Cohesion: 0.11
Nodes (40): DraftAnswer, BaseModel, 구조 검증만 한다: 인용 ID가 실제 제공된 근거를 가리키는지, action별로 요구되는 필드가 채워졌는지. 문장 내용이 근거와 의미적으로…, validate_draft(), Retrieve evidence and the matching corpus timestamp in one stage., retrieve_question_evidence(), _draft_from_dict(), _hit_from_dict() (+32 more)
...
Cohesion: 0.10
Nodes (36): PostgresLegalRepository, CorpusSearchUnavailableError, RuntimeError, Raised when the current corpus generation is not safe to search., _ConnectionContext, _document(), _FakeConnection, _FakeEngine (+28 more)
...
Cohesion: 0.06
Nodes (56): Compatibility facade and CLI for the v2 LlamaIndex ingestion pipeline. The…, Readable stages for the v2 LlamaIndex ingestion pipeline., _changed_provisions(), GenerationIngestionService, _has_unchanged_source(), IncrementalIngestionService, IngestionResult, _mark_generation_failed() (+48 more)
...
Cohesion: 0.07
Nodes (37): AnswerAction, post, Request, Return allowed legal-search results from the v1 repository., search(), derive_fallback_action(), AiFailureCategory, AiFallbackReason (+29 more)
...
Cohesion: 0.17
Nodes (34): build_blocked_route_messages(), build_core_messages(), build_messages(), build_messages_v2(), CoreDraft, QuestionRoute, 0043: 법률을 처음 접하는 사용자를 위한 문체 규칙을 추가한 v2 프롬프트. 인용·근거·action 안전 규칙은…, Build the first v2 generation prompt without requesting unpublished detail. (+26 more)

### Community 45 - "ROADMAP.md"
Cohesion: 0.04
Nodes (38): 언제 더 읽어야 하는가, 이 파일을 갱신하는 시점, 지금 무엇이 진행 중인가, 현재 상태 (세션 시작 포인터), 활성 실행 계획, 0038: 모델 호출 없는 API는 전부 1초 이내 응답, 구현 결과 (2026-08-09), 비범위 (+30 more)
...
Cohesion: 0.05
Nodes (46): PhaseRequest, PrepareQuestion, Validated transport input needed to create or replay an execution., Validated transport ownership input for a core or finalize phase., citations_for_hits(), execution_generation_hits(), freeze_citations(), Any (+38 more)
...
Cohesion: 0.10
Nodes (23): _apply_prepared_transaction(), _BoundEngine, _BoundTransactionContext, _chunks(), current_corpus_snapshot_id(), _embedding_source_sha256(), AsyncConnection, T (+15 more)
...
Cohesion: 0.23
Nodes (15): _configure_ai(), _FailingRouter, _hit(), _payload_json(), asyncio, _SlowRouter, test_legal_search_runs_generation_and_validation_after_retrieval(), test_malformed_unavailable_draft_uses_deterministic_empty_fallback() (+7 more)
...
Cohesion: 0.11
Nodes (30): 국가법령정보 Open API 전용 독립 수집기., T, resolve(), PreparedUpdateRepository, preview_has_corpus_changes(), preview_source_deletions(), Protocol, UUID (+22 more)
...
Cohesion: 0.11
Nodes (25): ConsentRequiredError, Exception, AsyncClient, Exception, UUID, SupabaseAuth, SupabaseAuthError, SupabaseAuthUnavailableError (+17 more)
...
Cohesion: 0.06
Nodes (25): LlamaIndexLegalRepository, date, datetime, UUID, 단일 조문 조회를 v1 저장소에 위임한다., Corpus 항목 상태 조회를 v1 저장소에 위임한다., Corpus 검색 상태 조회를 v1 저장소에 위임한다., Corpus 기준일 범위 상태 조회를 v1 저장소에 위임한다. (+17 more)

### Community 77 - "main.py"
Cohesion: 0.04
Nodes (66): bind_app_dependencies(), _FactoryCompositionMain, Any, Shared transport dependencies that preserve ``app.main`` test seams., Module-like request facade that binds routes to one app factory's adapters., Bind a non-production app factory's resources for one HTTP request., Restore the previous factory binding after a request completes., reset_app_dependencies() (+58 more)
...
Cohesion: 0.06
Nodes (94): _acquire_corpus_mutation_lock(), _acquire_corpus_sync_run_lock(), _append_cache(), _arguments(), _backfill_database(), _bundle_passages(), _cache_batch_values(), _cache_file_lock() (+86 more)
...
### Community 85 - "NvidiaNimAnswerer"
Cohesion: 0.12
Nodes (16): NvidiaNimAnswerer, Any, QuestionRoute, NVIDIA hosted NIM adapter with a schema-validated legal answer boundary., 0046: 사전 라우팅이 legal_search 밖으로 걸러낸 질문(embedding·검색 없음)에 근거 없이 LLM을 호출한다 -…, Release the process-owned NVIDIA HTTP client., _answerer_for(), load_cases() (+8 more)

### Community 86 - "test_prepared_update.py"
Cohesion: 0.11
Nodes (14): CurrentEmbeddingSource, _Client, _Connection, _ConnectionContext, _document(), _Engine, asyncio, Path (+6 more)
...
Cohesion: 0.15
Nodes (13): NvidiaNimQuestionRouter, Any, BaseModel, Question router backed by one structured NVIDIA NIM request., Release the process-owned NVIDIA HTTP client., _RouteJudgmentSchema, _git_sha(), load_d10_cases() (+5 more)
...
Cohesion: 0.11
Nodes (31): korea_today(), date, ValueError, Dynamic temporal contract for the currently searchable legal corpus. The…, Return the product's legal-current date, independent of server timezone., Raised when a request falls outside the current dynamic corpus bounds., Return a supported date or fail before quota and provider work begins., require_supported_corpus_date() (+23 more)
...
Cohesion: 0.12
Nodes (11): build_v1_query_embedding_capability(), LegacyQueryEmbeddingCapability, Keep framework-specific v1 embedding policy at the composition boundary., Composition-owned policy for v1 query-vector generation., Compatibility facade for callers that still provide a repository object., _requires_legacy_query_embedding(), LegalRepository, date (+3 more)
...
Cohesion: 0.06
Nodes (58): main_module(), MockUser, QuestionResponse, Resolve the composition entry lazily to retain monkeypatch compatibility., _save_if_authenticated(), cancel_question_execution(), _capability_hash(), _execution_capability() (+50 more)
...
Cohesion: 0.06
Nodes (51): clarification_resubmission_summary(), post_generation_clarification_answer(), QuestionResponse, 0028 "비용 최소화 결정"의 재제출 템플릿. route_guidance_fallback(사전 라우팅)와…, 2026-08-08: DraftAnswer.action == "clarification_required"일 때 쓴다 - 사전 라우팅이 못 잡고…, Build the deterministic AI-mode fallback for a route without evidence.…, route_guidance_fallback(), T (+43 more)
...
### Community 123 - "V2 LlamaIndex 검색(Retrieval) 파이프라인 구현 계획"
Cohesion: 0.11
...
Cohesion: 0.42
Nodes (8): derive_answer_action(), ChecklistItem, _item(), ChecklistItem, test_all_required_or_not_applicable_is_fully_answerable(), test_any_check_status_is_clarification_required(), test_any_conditional_without_check_is_partially_answerable(), test_empty_checklist_is_unanswerable()
...
### Community 131 - "RouteJudgment"
Cohesion: 0.16
Nodes (8): A provider judgment for one of the four provider-resolvable routes., RouteJudgment, _ProviderTimeoutRouter, _LegalRouter, test_disabled_search_only_feature_reports_ai_unavailability_without_search_only_warning(), test_disabled_search_only_feature_returns_503_after_generation_failure(), test_search_only_feature_defaults_to_disabled_and_can_be_enabled(), TimeoutError

### Community 132 - "PostgresQuestionExecutionRepository"
Cohesion: 0.12
Nodes (27): datetime, ExecutionStatus, UUID, Persist a phase result and every public event under one lock. A provider call…, StoredQuestionExecution, _json_mapping(), PostgresQuestionExecutionRepository, AsyncEngine (+19 more)

### Community 133 - "test_prepared_publisher_postgres.py"
Cohesion: 0.19
Nodes (14): _assert_writer_locks_released(), _async_url(), _bundle(), _complete(), _isolated_repository(), _no_sleep(), asyncio, parametrize (+6 more)
...
Cohesion: 0.18
Nodes (17): All collaborator ports for a single v2 service instance. The composition root…, V2ExecutionDependencies, build_app_dependencies(), build_nvidia_answerer(), build_nvidia_embedder(), build_nvidia_question_router(), build_v2_execution_dependencies(), Settings (+9 more)
...
Cohesion: 0.07
Nodes (36): is_allowed_source_url(), 브라우저에 노출 가능한 국가법령정보 원문 URL만 허용한다., emit_execution_phase(), emit_question_outcome(), emit_question_stage_timing(), emit_route_outcome(), ExecutionPhaseEvent, fallback_reason_metrics_snapshot() (+28 more)
...
Cohesion: 0.13
Nodes (23): FinalAnswer, FinalAnswerCoordinator, Choose one authoritative terminal response from already verified content., VerifiedAnswer, Verify every sentence independently so no unchecked claim leaks into SSE., text_is_grounded(), EventProtocolError, ValueError (+15 more)
...
Cohesion: 0.19
Nodes (8): Backward-compatible imports for the active-generation query cache., ActiveGenerationIndexProvider, ActiveIndex, Request-safe cache for indexes opened from the active generation pointer., One index paired with the immutable generation it reads., Cache one index per active generation without changing prior pins., Resolve the current pointer once and return an immutable request pin., Release the caller-owned database engines, if this provider owns them.
...
### Community 231 - "questions.py"
Cohesion: 0.15
Nodes (21): _optional_user(), _answering_http_error(), cancel_question(), _handle_question(), HTTPException, post, QuestionResponse, QuestionStageTimingOutcome (+13 more)

### Community 232 - "FakePostgresIdentity"
Cohesion: 0.24
Nodes (3): FakePostgresIdentity, MockUser, UUID
...
### Community 374 - "단계 구조"
Cohesion: 0.18
Nodes (11): Task 10: Whole-workspace verification and completion, Task 1: Generation·execution persistence migration, Task 2: Generation-aware ingestion and atomic publish, Task 3: Pinned active index and router/query-engine adapter, Task 4: Pure execution domain, grounding and final-answer contract, Task 5: Execution repository and global capacity lease, Task 6: Preparation service, phase coordinator, grounded streams, Task 7: FastAPI prepare/core/finalize routes and removal of the v2 single route (+3 more)
...
Cohesion: 0.08
Nodes (44): MemoryQuestionExecutionRepository, In-memory reference implementation of the authoritative execution contract., PhaseResult, datetime, QuestionPhaseCoordinator, Authoritative, replay-safe v2 phase coordination. Provider work is supplied as…, Start exactly one phase or return its persisted authoritative replay., Validate a core draft before persisting the only publishable core event. (+36 more)
...
### Community 392 - "select_generation_hits"
Cohesion: 0.50
...
Cohesion: 0.15
Nodes (11): v2 LlamaIndex 검색과 v1 저장소 위임을 결합한다., asyncio, test_non_search_methods_forward_concrete_arguments_to_delegate(), test_search_returns_hits_from_this_adapter_search_with_trace(), test_search_with_trace_pins_the_active_generation_store_for_one_request(), test_search_with_trace_uses_v2_retriever_and_returns_dense_trace(), CollectorRepository, Any (+3 more)
...
Cohesion: 0.33
Nodes (13): MockUser, SupabaseIdentity, _existing(), FakeEngine, _identity(), asyncio, test_delete_history_locks_conversation_before_deleting_question(), test_existing_consented_profile_does_not_require_headers_again() (+5 more)
...
### Community 425 - "실험 D-10 M3 — raw/R1 소표본 calibration 요약"
Cohesion: 0.10
Nodes (16): 0029: 필요 시 D-full Gold 제작, active 승격 조건, 목적, 보존 자산, 완료 조건, M3 step 5 — R1 새 top 5 미판정 후보 해소, raw vs R1 (v3 Gold 기준, 최종), 문항별 결정 (+8 more)
...
Cohesion: 0.15
Nodes (12): 0041: 법제처 API의 법종구분코드를 실제로 파싱해 저장·응답에 반영, 결정 사항 (사용자, 2026-08-08), 구현 결과 (2026-08-09), 비범위, 승격 조건, 완료 조건, 확인된 사실, 후속 결정: `source_kind`·`law_type_code` 통합 보류 (2026-08-09) (+4 more)
...
### Community 464 - "execution_request_and_hits"
Cohesion: 0.67
Nodes (3): execution_request_and_hits(), datetime, Read the persisted prepare payload without triggering a second retrieval.
...
- **Why does `PostgresLegalRepository` connect `PostgresLegalRepository` to `bootstrap.py`, `evaluate_experiment_d_gold.py`, `postgres_repository.py`, `test_experiment_d_gold_preflight.py`, `main.py`, `search_only_answer`, `SearchTrace`, `PostgresExperimentDBackend`, `test_backfill_embeddings.py`, `SourceKind`, `LegalDocumentRecord`, `get_settings`, `CorpusTemporalState`?**
  _High betweenness centrality (0.034) - this node is a cross-community bridge._
- **Why does `SourceKind` connect `SourceKind` to `experiment_search.py`, `publish_prepared_bundle`, `RouteJudgment`, `run.py`, `test_prepared_publisher_postgres.py`, `RawResponse`, `law_xml.py`, `postgres_repository.py`, `corpus_update_bundle.py`, `MemoryLegalRepository`, `test_question_timeout_budget.py`, `test_api_factory_composition.py`, `LawOpenApiClient`, `LegalDocumentRecord`, `SearchHit`, `PostgresLegalRepository`, `DeletionRecord`, `law_rag_core/domain/schemas.py`, `test_security_boundaries.py`, `QuestionRequest`, `test_corpus_update_bundle.py`, `test_routing_pipeline.py`, `law_rag_collector/service.py`, `law_rag_core/domain/catalog.py`, `MockCorpusRepository`, `search_only_answer`, `test_backfill_embeddings.py`, `account.py`, `test_prepared_update.py`, `test_layperson_prompt_v2.py`, `law_json.py`, `test_question_cancellation.py`, `main_module`, `MemoryQuestionExecutionRepository`?**
  _High betweenness centrality (0.023) - this node is a cross-community bridge._
- **Why does `main_module()` connect `main_module` to `RouteJudgment`, `build_v1_answer_dependencies`, `MemoryLegalRepository`, `test_question_timeout_budget.py`, `test_api_factory_composition.py`, `sse.py`, `SearchHit`, `law_rag_core/domain/schemas.py`, `test_security_boundaries.py`, `legal_search_router`, `test_routing_pipeline.py`, `SupabaseAuthError`, `LlamaIndexLegalRepository`, `main.py`, `account.py`, `CorpusTemporalState`, `test_question_cancellation.py`, `questions.py`, `MemoryQuestionExecutionRepository`, `test_v2_search.py`?**
