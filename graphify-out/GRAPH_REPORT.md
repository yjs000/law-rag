...
## Corpus Check
- 602 files · ~501,085 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 8009 nodes · 15733 edges · 509 communities (423 shown, 86 thin omitted)
- Extraction: 92% EXTRACTED · 8% INFERRED · 0% AMBIGUOUS · INFERRED: 1267 edges (avg confidence: 0.91)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `1da5e3be`
...
## Communities (509 total, 86 thin omitted)
...
Cohesion: 0.16
Nodes (40): embedding_text_sha256(), SourceProvision, canonical_gold_corpus_snapshot_id(), Hash unique content populations; evaluation dates are sealed separately., audit_gold_dataset(), question_set_sha256(), _source_bank_binding_errors(), _approval_manifest() (+32 more)
...
Cohesion: 0.11
Nodes (46): test_content_snapshot_identity_does_not_include_the_calendar_date(), BundleState, _atomic_write(), _build_manifest(), canonical_corpus_population_fingerprint(), canonical_corpus_publish_snapshot_id(), canonical_corpus_snapshot_id(), _canonical_json() (+38 more)

### Community 13 - "MemoryLegalRepository"
Cohesion: 0.07
Nodes (44): _date_or_none(), MemoryLegalRepository, date, Path, UUID, CorpusSearchUnavailableError, RuntimeError, Raised when the current corpus generation is not safe to search. (+36 more)
...
Cohesion: 0.10
Nodes (49): _contains_normative_assertion(), DraftAnswer, _evidence_for_citations(), BaseModel, 구조 검증만 한다: 인용 ID가 실제 제공된 근거를 가리키는지, action별로 요구되는 필드가 채워졌는지. 문장 내용이 근거와 의미적으로…, 근거와 겹치는 용어 비율(>=50%)을 요구해 무근거 주장을 막는다. 2026-08-08: `unanswerable` action의…, _strip_epistemic_hedges(), _terms() (+41 more)
...
Cohesion: 0.10
Nodes (21): ApprovalManifestSourceBank, ApprovedQuestion, ExperimentDGoldAdjudicationManifest, GoldAdjudicatedCase, GoldAnnotationProtocol, GoldAnnotationReview, GoldAsOfPopulation, GoldCorpusSnapshot (+13 more)
...
Cohesion: 0.06
Nodes (45): changes(), corpus_status(), health(), provision(), date, get, post, Request (+37 more)
...
Cohesion: 0.07
Nodes (67): NvidiaNimAnswerer, Any, QuestionRoute, NVIDIA hosted NIM adapter with a schema-validated legal answer boundary., 0046: 사전 라우팅이 legal_search 밖으로 걸러낸 질문(embedding·검색 없음)에 근거 없이 LLM을 호출한다 -…, Release the process-owned NVIDIA HTTP client., build_blocked_route_messages(), build_core_messages() (+59 more)

### Community 45 - "ROADMAP.md"
Cohesion: 0.04
Nodes (45): 언제 더 읽어야 하는가, 이 파일을 갱신하는 시점, 지금 무엇이 진행 중인가, 현재 상태 (세션 시작 포인터), Python docstring 정책, 결정 기록, 규칙, 목적 (+37 more)
...
Cohesion: 0.06
Nodes (36): PhaseRequest, PrepareQuestion, Validated transport input needed to create or replay an execution., Validated transport ownership input for a core or finalize phase., citations_for_hits(), execution_generation_hits(), execution_request_and_hits(), freeze_citations() (+28 more)
...
Cohesion: 0.06
Nodes (46): NvidiaNimQuestionRouter, Any, BaseModel, Release the process-owned NVIDIA HTTP client., Question router backed by one structured NVIDIA NIM request., _RouteJudgmentSchema, Protocol, QuestionRouter (+38 more)
...
Cohesion: 0.06
Nodes (21): AI 차별화와 안전 설계, 결정 기록, 제품 표현 원칙, 핵심 신념, GitHub 이슈와 PR 운영, PR, 이슈, 프로젝트 보드 (+13 more)
...
Cohesion: 0.10
Nodes (33): 국가법령정보 Open API 전용 독립 수집기., T, resolve(), CurrentEmbeddingSource, PreparedUpdateRepository, preview_has_corpus_changes(), preview_source_deletions(), Protocol (+25 more)

### Community 64 - "compilerOptions"
Cohesion: 0.07
Nodes (26): compilerOptions, allowJs, esModuleInterop, incremental, isolatedModules, jsx, lib, module (+18 more)

### Community 65 - "DeletionRecord"
Cohesion: 0.16
Nodes (10): DeletionRecord, CollectorRepository, Any, date, Protocol, DeletionClient, asyncio, _seed() (+2 more)
...
Cohesion: 0.06
Nodes (43): LlamaIndexLegalRepository, date, datetime, UUID, v2 LlamaIndex 검색과 v1 저장소 위임을 결합한다., 단일 조문 조회를 v1 저장소에 위임한다., Corpus 항목 상태 조회를 v1 저장소에 위임한다., Corpus 검색 상태 조회를 v1 저장소에 위임한다. (+35 more)

### Community 77 - "api/dependencies.py"
Cohesion: 0.14
Nodes (18): ConsentRequiredError, Exception, _bearer_token(), _optional_user(), MockUser, QuestionResponse, Shared transport dependencies that preserve ``app.main`` test seams., _require_mock_auth() (+10 more)
...
Cohesion: 0.09
Nodes (23): PhaseResult, datetime, QuestionPhaseCoordinator, Authoritative, replay-safe v2 phase coordination. Provider work is supplied as…, Start exactly one phase or return its persisted authoritative replay., Explicit ports used by the v2 question-execution use case., All collaborator ports for a single v2 service instance. The composition root…, V2ExecutionDependencies (+15 more)
...
Cohesion: 0.15
Nodes (17): PreparedExecution, Prepared execution plus the anonymous replay capability, if any., core_degraded_response(), core_is_grounded(), grounding_fallback(), Any, QuestionResponse, SDK-independent grounding predicates and safe v2 fallback construction. (+9 more)
...
### Community 93 - "test_v2_question_executions.py"
Cohesion: 0.21
...
Cohesion: 0.11
Nodes (27): korea_today(), date, ValueError, Dynamic temporal contract for the currently searchable legal corpus. The…, Return the product's legal-current date, independent of server timezone., Raised when a request falls outside the current dynamic corpus bounds., Return a supported date or fail before quota and provider work begins., require_supported_corpus_date() (+19 more)
...
Cohesion: 0.03
Nodes (114): bind_app_dependencies(), _FactoryCompositionMain, main_module(), Any, Module-like request facade that binds routes to one app factory's adapters., Bind a non-production app factory's resources for one HTTP request., Restore the previous factory binding after a request completes., Resolve the composition entry lazily to retain monkeypatch compatibility. (+106 more)
...
Cohesion: 0.04
Nodes (94): _answering_http_error(), cancel_question(), _handle_question(), HTTPException, post, QuestionResponse, QuestionStageTimingOutcome, Request (+86 more)
...
### Community 125 - "debug_retrieval_pipeline.py"
Cohesion: 0.18
...
Cohesion: 0.05
Nodes (59): NvidiaNimEmbedder, NVIDIA hosted NIM embedding adapter with the existing batch contract., Release the process-owned NVIDIA HTTP client., Keep at most one ranked leaf per article within the provider input budget., select_generation_hits(), Protocol, QueryEmbeddingCapability, Declare whether this retrieval path needs an application query vector. (+51 more)
...
### Community 170 - "실행 계획 0027: 실험 D-10-R1 로컬 재정렬"
Cohesion: 0.20
...
### Community 180 - "ActiveGeneration"
Cohesion: 0.25
...
Cohesion: 0.12
Nodes (39): _authenticated_user(), Header, conversation_turns(), conversations(), current_user(), _decode_conversation_cursor(), _decode_cursor(), _decode_turn_cursor() (+31 more)
...
### Community 251 - "canonical_gold_dataset_sha256"
Cohesion: 0.33
...
### Community 357 - "실험 D — 검색 문맥 안전 게이트 평가"
Cohesion: 0.29
Nodes (4): 결과, 실험 D — 검색 문맥 안전 게이트 평가, 판정, 한계

### Community 358 - "test_memory_retrieval_quality.py"
Cohesion: 0.67
...
Cohesion: 0.14
Nodes (23): _admit_v2_provider_phase(), Acquire provider capacity before sending an SSE response when work will start., PhaseDeadline, datetime, ExecutionStatus, asyncio, fixture, repository() (+15 more)
...
Cohesion: 0.06
Nodes (33): RAG 파이프라인 설계, 검색 단계, 결정 기록, 답변 계약, 목표, 문서 모델, 실패 모드, 캐시와 재현성 (+25 more)

### Community 396 - "프로젝트 로드맵"
Cohesion: 0.50
Nodes (4): Blocked, Done, Todo, 프로젝트 로드맵
...
### Community 401 - "Python Docstrings and Ruff D Implementation Plan"
Cohesion: 0.50
Nodes (4): Python Docstrings and Ruff D Implementation Plan, 검증 증거, 결과, 알려진 환경 제약
...
Cohesion: 0.10
Nodes (17): 0058: v2 청킹 ablation — 현재 조문 노드 vs LlamaIndex 하위 청킹, active 승격 조건, 결정 기록, 고정 조건, 목표, 범위, 비범위, 완료 조건 (+9 more)
...
- **Why does `PostgresLegalRepository` connect `PostgresLegalRepository` to `bootstrap.py`, `law_rag_core/domain/schemas.py`, `main.py`, `evaluate_experiment_d_gold.py`, `postgres_repository.py`, `SearchTrace`, `search_only_answer`, `PostgresExperimentDBackend`, `test_backfill_embeddings.py`, `LegalDocumentRecord`, `SourceKind`, `preflight_experiment_d_gold.py`, `get_settings`, `debug_retrieval_pipeline.py`, `CorpusTemporalState`?**
  _High betweenness centrality (0.033) - this node is a cross-community bridge._
- **Why does `SourceKind` connect `SourceKind` to `experiment_search.py`, `publish_prepared_bundle`, `render_pdf`, `run.py`, `test_prepared_publisher_postgres.py`, `RawResponse`, `ProvisionRecord`, `postgres_repository.py`, `corpus_update_bundle.py`, `MemoryLegalRepository`, `test_question_timeout_budget.py`, `test_api_factory_composition.py`, `LegalDocumentRecord`, `LawOpenApiClient`, `SearchHit`, `PostgresLegalRepository`, `deletions.py`, `law_rag_core/domain/schemas.py`, `law_json.py`, `QuestionRequest`, `test_corpus_update_bundle.py`, `RouteJudgment`, `law_rag_collector/service.py`, `DeletionRecord`, `law_rag_core/domain/catalog.py`, `SearchTrace`, `search_only_answer`, `test_backfill_embeddings.py`, `test_prepared_update.py`, `test_v2_question_executions.py`, `test_memory_retrieval_quality.py`, `main.py`, `v1/answering.py`, `MockCorpusRepository`?**
  _High betweenness centrality (0.028) - this node is a cross-community bridge._
- **Why does `main_module()` connect `main.py` to `MemoryLegalRepository`, `test_question_timeout_budget.py`, `test_api_factory_composition.py`, `sse.py`, `SearchHit`, `bootstrap.py`, `law_rag_core/domain/schemas.py`, `legal_search_router`, `RouteJudgment`, `test_supabase_authenticated_flow.py`, `SearchTrace`, `api/dependencies.py`, `account.py`, `test_v2_question_executions.py`, `CorpusTemporalState`, `test_search_only_feature.py`, `v1/answering.py`, `ExecutionStatus`, `test_v2_search.py`?**
