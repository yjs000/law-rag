...
- **남은 사용자 조치:** 없음.
- **상태:** `Resolved` — D-003에서 active index와 계획 lifecycle 정리를 완료함.
...
- **검증:** 수정 전 import 해결만으로는 async 4건 실패를 재현했고, 최종 PR #1의 GitHub-hosted Python job에서 PostgreSQL integration 포함 212건과 collector·문서 gate가 통과함.
...
- **검증:** PostgreSQL 16·17 임시 container와 보존된 CI 통합 테스트에서 동시 저장·명시적 export 정리·대화 재집계·ACL·미래 cutoff·멱등성·downgrade가 통과함. CI는 축약 `0005` 호환 schema에 실제 `0006` SQL을 적용하며 전체 migration chain 실행을 주장하지 않음.
...
- **상태:** `Resolved` — immutable review 승인과 원격 Python/Web CI 통과. Production 적용은 별도 D-009로 추적함.
...
- **남은 사용자 조치:** 없음.
- **상태:** `Resolved` — immutable review 승인과 원격 Python CI 통과.

### 9. 사용자 단일 이력 삭제와 retention의 교차 deadlock

- **원문/관찰:** 독립 재review가 `delete_history()`의 question-first 삭제와 retention의 conversation-first 잠금이 교차해 PostgreSQL `40P01`을 발생시키는 경로를 재현함.
- **원인:** 새 질문 저장과 retention은 conversation-first였지만 사용자 단일 이력 삭제만 question → conversation 순서를 사용함.
- **영향:** 동일 대화에서 사용자 삭제와 retention이 겹치면 retention 실행이 실패할 수 있음. Production 미적용 상태라 실제 사용자 데이터 영향은 없음.
- **해결:** `delete_history()`가 소유권을 확인하며 conversation을 `FOR UPDATE OF c`로 먼저 잠근 뒤 질문을 삭제·대화 요약 정리하도록 변경함.
- **검증:** PostgreSQL 17에서 사용자 삭제가 conversation lock을 보유한 동안 retention이 대기하고, 삭제 commit 뒤 `40P01` 없이 성공함을 확인함. 빈 대화 삭제와 동시 export 사용자 삭제의 실제 count도 통합 테스트로 고정함.
- **재발 방지:** conversation과 question을 함께 변경하는 모든 write path는 conversation-first 잠금 순서를 단위·PostgreSQL 통합 테스트로 유지한다.
- **남은 사용자 조치:** 없음.
- **상태:** `Resolved` — immutable review 승인과 원격 Python CI 통과.

## 검증 체크리스트

- [x] 사건별 날짜, 관찰, 원인, 영향, 해결/완화, 검증, 재발 방지, 상태가 있다.
- [x] credential과 개인 원문은 `REDACTED` 처리했다.
- [x] 새 증거는 중복 사건 대신 기존 사건에 반영했다.
- [x] `Open`·`Mitigated` 사건의 남은 조치를 명시했다.
- [x] Markdown 링크와 `git diff --check`를 검증했다.
