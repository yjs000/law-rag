...
1보다 작다. `u = v[:512] / ||v[:512]||₂`로 바꾸면 남은 512개 좌표 사이의 비율과 **512차원
공간 안에서의 방향**은 유지하면서 norm만 1로 복원한다. 원래 2048차원 방향과 삭제한 1536개 좌표의
정보가 유지된다는 뜻은 아니다. NVIDIA 모델 카드도 similarity scoring 전에 이 재정규화를 요구한다.
