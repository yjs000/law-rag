...
상태: `제안됨 · 미착수 — 실 트래픽/tracking 데이터 축적이 선행 조건`
...
- `scripts/build_tier1_term_dictionary.py`가 v1 질문은행 1,000문항 중 BUILD 200개만 분석했다
  (2026-08-08, [0028](0028-pre-retrieval-question-routing.md) 참고) - 나머지 EVAL 800개는
...
- **재검토 시 할 일**: EVAL 800개 + 실제 tracking 데이터를 함께 보고 새 키워드/패턴 후보를 다시
  채굴한다. 자동 빈도 추출을 최종 판단으로 쓰지 않고 사람이 직접 읽고 채택하는 원칙
  (2026-08-08 tier1 사전 확장 때 확립)을 그대로 따른다 - 특히 `boundary-document-keyword-
  false-positive`·`boundary-realtime-keyword-false-positive` 같은 위양성 패턴을 다시
  점검한다.
...
- **비인증 사용자**: 개인정보 불변조건상 질문 원문·설명 텍스트는 저장하지 않는다(의도된 설계,
  ...
  분포에서도 비슷한 비율로 나타나는지).
- **재검토 시 할 일**: 인증 사용자 표본을 D-10 방식으로 사람이 검토해 tier1/tier2 오분류 패턴을
  찾고, 같은 기간 비인증 집계 분포(`route_metrics_snapshot()`,
  `fallback_reason_metrics_snapshot()`)와 비교해 일반화 가능한 패턴인지 확인한다.

## 승격 조건

- 다음 중 하나 이상이 실제로 관측됐을 때: (a) `_route_by_route_tier`에서 tier2 호출 비율이
  유의미하게 누적됐다, (b) 인증 사용자 질문 이력이 D-10 표본 검토를 할 만큼 쌓였다, (c) 사용자가
  명시적으로 재검토를 요청했다.
...
- tier1 사전 변경분이 있다면 `route-fixture-v1.json` 평가로 misclassification_rate 변화를
  기록한다(2026-08-08 타이트닝 때와 같은 방식).
