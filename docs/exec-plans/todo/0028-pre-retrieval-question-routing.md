...
3. **소형 LLM classifier(최후 수단)** — 1·2단계 모두 애매하면 그때만 작은 분류 전용 호출을 한다.
   전체 corpus나 design doc를 주지 않고 질문 원문 + 4개 route 정의 10줄 미만만 준다. 이 호출 자체는
   법률 답변 생성 모델과 분리하고, 결과(route, 확신도, 근거)를 관측 로그에 남긴다.
