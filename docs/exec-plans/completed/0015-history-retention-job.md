...
상태: 완료(2026-08-18) — migration·`SECURITY DEFINER` 정리 함수·감사 테이블 구현과 로컬 검증은
끝났다. 사용자가 Production `pg_cron`/외부 scheduler 등록은 하지 않기로 결정해, scheduler 배선과
최초 운영 실행 감사(구현 단계 5~8)는 착수하지 않고 이 계획을 종료한다. scheduler 없이 이력이
계속 쌓이는 상태는 알려진 결과로 남기며, 필요해지면 새 계획으로 scheduler 배선을 다시 연다.
