...
- [x] **Step 1: 로그인·가입 모달과 Google 인증 완료를 검증한다.**
- [x] **Step 2: 연속 질문, 새 질문, 새로고침 복원, 기록 열기와 페이지네이션을 검증한다.**
- [x] **Step 3: 기록 삭제 취소·확인, PDF 내보내기, 로그아웃을 검증한다.**
- [x] **Step 4: 실패가 있으면 TDD 수정 루프를 돌리고 인증 P0 전체를 다시 실행한다.**
- [x] **Step 5: P0 전체 완료 체크를 실행하고 P0가 모두 통과한 경우에만 Task 4로 이동한다.**
...
- [x] **Step 1: 입력, IME, 길이, XSS, 날짜와 필터 경계를 검증한다.**
- [x] **Step 2: 빈 결과, 불완전 조문, 범위 밖 질문, 인용 무결성, AI 폴백을 검증한다.**
- [x] **Step 3: 재시도 가능·불가능 HTTP 상태, 스트림 재연결, 중지, 화면 전환 경쟁 상태를 검증한다.**
- [x] **Step 4: 인증 취소·세션 만료·다른 탭 로그아웃·이력 및 내보내기 실패를 검증한다.**
- [x] **Step 5: 각 실패마다 TDD 수정 후 P1 관련 묶음과 P0 정상 흐름을 함께 재실행한다.**
...
- [x] **Step 1: 375px, 430px, 1280px에서 동일 핵심 시나리오를 캡처하고 overflow·가림을 확인한다.**
- [x] **Step 2: 모달 포커스 순환, Escape 복귀, 키보드 조작, live region을 검증한다.**
- [x] **Step 3: `pnpm test:web`, `pnpm lint:web`, `pnpm typecheck`, `pnpm build:web` 및 관련 API 회귀를 실행한다.**
- [x] **Step 4: 코드 변경이 있으면 `graphify update .`를 실행하고 변경 산출물을 함께 검증한다.**
- [x] **Step 5: 독립 리뷰를 반영하고 모든 명세 체크박스가 증거와 함께 완료됐는지 확인한다.**
- [x] **Step 6: 계획을 Done으로 옮기고 roadmap renderer/checker를 실행한 뒤 기능 단위로 커밋한다.**
...
- 2026-09-04 KST production에서 정상 추천 질문과 같은 대화의 후속 질문이 인용·체크리스트·원문 확장·새로고침 복원까지 완료됐다.
- V2 근거 경계 API 회귀 38건과 웹 테스트 104건, 린트, 타입 검사, 프로덕션 빌드가 통과했다.
- 반응형·대화상자 키보드 점검에서 찾은 접근성 결함은 `c768bf2`, `5b01889`로 수정·배포했다.

## 잔여 작업

- 없음. 다음 우선순위는 생성된 로드맵의 첫 Todo를 따른다.
