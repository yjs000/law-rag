...
- [x] `M3 — 문서와 전체 검증`: 학습 브리핑과 인덱스를 갱신하고 코어 테스트, Ruff, 문서 검사,
...
- 2026-07-22: `pnpm.cmd verify`에서 코어 4개, API 206개 통과·2개 환경 의존 테스트 건너뜀,
  collector 34개, web 46개 테스트와 Python/웹 lint, 타입 검사, 문서 검사, Next.js build가 통과했다.
- 2026-07-22: 별도 ad hoc 명령에서 코어 프로젝트 pytest에 저장소 루트 테스트를 암묵적으로
  선택해 API import 28건이 수집 실패했으나, 공식 프로젝트별 `pnpm.cmd verify`에서는 같은 API
  테스트 206개가 통과했다. 구현 실패가 아니라 잘못 지정한 테스트 범위였으며 공식 검증을 기준으로 한다.
