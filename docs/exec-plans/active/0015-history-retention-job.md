...
- [x] 삭제 질문의 `checklist_exports`는 기존 FK `ON DELETE CASCADE`로 삭제되고 수량을 기록한다.
- [x] 영향받은 대화만 재집계하며 남은 턴이 없으면 대화를 삭제한다.
...
3. [x] 함수 실행 권한을 `PUBLIC`에서 회수하고 `service_role`에만 부여한다.
...
`pg_cron`이 허용되지 않으면 Supabase가 지원하는 승인된 scheduler/Edge Function 경로를 사용하되 동일 함수를 호출한다. 어떤 경로도 애플리케이션 요청 트래픽에 정리를 의존시키지 않는다.
