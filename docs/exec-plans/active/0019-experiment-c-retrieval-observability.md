...
- [x] M3 — ask 자동 이력 기록 구현·테스트·커밋
- [x] M4 — 고정 평가셋·평가 명령·대표 결과 구현·실행·커밋
...
- 2026-07-23: 성공한 ask의 실제 stdout과 corpus/stdout SHA-256을 로컬 JSON 이력과 Markdown에
  원자 기록하고 실행 번호를 누적하도록 구현했다. 기록 실패 보존과 `--no-record`를 포함한 테스트
  16개와 Ruff가 통과했다.
- 2026-07-23: 고정 질문 6개와 `evaluate` 명령, Law@1·Article Recall@3/5/10·MRR을 구현했다. 실제
  dense 기준선은 범위 내 5문항에서 Law@1 `1.0`, Recall@3 `0.8`, Recall@5 `0.8`, Recall@10
  `1.0`, MRR `0.82`였다. 태양광 질문은 신재생에너지법 제2조가 raw·조 모두 1위였다.
- 2026-07-23: 실제 ask 1회를 실행해 로컬 이력 run 1, stdout hash 일치와 Markdown 내 exact stdout
  포함을 확인했다.
