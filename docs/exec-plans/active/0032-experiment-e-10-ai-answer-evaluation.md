...
상태: `계획 확정 · 실행 전 — 호출 수·비용·판정표 사전 등록 완료, 사용자 승인 대기`
...
2. **E-10 base — D-10 10문항 실제 hosted 실행**: 동결 M4 문맥(승자 R1+A)으로 10문항 각각 실제
   NVIDIA 답변 생성을 1회 호출하고, 라우팅(tier1/tier2)도 실제 파이프라인 그대로 통과시킨다.
...
- **라우팅**: tier1이 이미 D-10 4문항(`0251`·`0111`·`0605`·`0836`)을 무료로 즉시 처리한다
  (`route-fixture-v1-results.json` 확인). 나머지 6문항만 tier2 LLM 호출 대상 — **최대 6회**.
- **답변 생성**: `legal_search`로 라우팅되는 문항만 실제 생성 호출 대상. tier1이 잡은 4문항 중
  `0605`·`0836`(realtime)·`0251`·`0111`(clarification)은 라우팅 단계에서 차단 응답으로 끝나
  생성 호출이 없다. 나머지 6문항이 tier2에서 `legal_search`로 판정되면 그만큼만 생성 호출 —
  **최대 6회**.
- **총 상한**: 라우팅(tier2) 최대 6회 + 답변 생성 최대 6회 = **최대 12회** 실제 NVIDIA 호출.
  실제로는 tier2가 일부를 `clarification_required`/`realtime_required`/
