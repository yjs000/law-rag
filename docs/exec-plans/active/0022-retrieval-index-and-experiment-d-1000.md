...
- [ ] 현재 파서로 운영 corpus를 재수집하고 변경된 조문의 벡터와 v3 qrels를 다시 생성한다.
- [ ] 사용자가 수동 검토 11문항과 실험 D 질문 구성을 확인한다.
...
- v3 자동 생성 평가셋에서 분리한 11개 문항의 의미 적합성은 사용자가 직접 읽고 승인할 수 있다. 이는 데이터셋 생성 완료를 막는 조건이 아니라 수동 품질 등급을 높이기 위한 검토 큐다.
...
- 운영 corpus 3,066개에 현재 NVIDIA 512차원 passage 벡터와 partial HNSW 인덱스가 준비됐다.
- 실험 D v3 검토 초안은 고정 qrels를 가진 1,000문항, calibration 200/test 800, answerable 850/unanswerable 150, 수동 검토 11개로 생성됐다. 질문 구성은 사용자 확인 대기다.
