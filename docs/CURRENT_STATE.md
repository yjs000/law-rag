...
현재 `Picked Up` milestone은 없다. F-006 Web grounded QA 프런트 API 정합성을 완료했으며, 기준일 입력은 한국 today() 상한을 다음 한국 자정에 갱신하고 선택값을 고정된 V2 실행 API에 전달한다. 체크리스트 내보내기 프런트 제거는 후속 F-007에 남아 있다. 이후 우선순위는 [docs/ROADMAP.md](ROADMAP.md)에서 확인한다. 로드맵은 세션 시작용 얇은 색인이며 모든 실행계획을 읽으라는 뜻이 아니다.
