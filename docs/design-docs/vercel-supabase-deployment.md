...
`/v1/questions` 조정된 예산과 직접 연결된 여섯 개 비밀 아님 timeout 변수는 아래와 같다. 값의 원본은 `apps/api/.env.example`이며 이 표는 각 변수의 역할만 옮긴다.
...
| `EMBEDDING_TIMEOUT_SECONDS` | 5 | 질문 임베딩 stage 상한 |
