...
3. 별도 watcher가 250~500ms 간격으로 해당 한 행의 `cancel_requested_at`만 조회한다.
...
- watcher polling은 활성 질문당 DB 부하를 추가한다. 500ms에서 동시 100건이면 최대 약 200 read/s이므로 실제 동시성과 Supabase 한도를 측정해 250ms~1초 사이를 조정한다.
