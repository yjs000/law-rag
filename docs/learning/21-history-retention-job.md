...
승인된 scheduler 또는 수동 운영 호출
  ...
  -> 영향받은 conversations를 저장 경로와 같은 순서로 잠금
  -> checklist_exports DELETE ... RETURNING으로 실제 삭제 수 집계
  -> question_history 삭제
       -> 남은 checklist_exports는 FK cascade 안전망
...
함수가 실패를 다시 raise하면 같은 transaction의 감사 행도 rollback된다. 현재 계약은 실패 감사 행을 commit하고 반환하므로 외부 scheduler가 `status='failed'`를 반드시 job failure로 변환한다. 단순 SQL 성공만 보는 scheduler는 별도 마지막 성공·연속 실패 monitor 없이는 사용할 수 없다.
