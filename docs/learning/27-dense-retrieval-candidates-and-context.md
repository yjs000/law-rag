...
따라서 실험 C는 후보 10개를 관찰하고, 실험 D가 그중 직접 근거 1~5개만 문맥으로 만든다. `top 10`은
AI에 그대로 넘길 문서 수가 아니다.
...
1. dense-only 기준선을 고정한다.
2. 같은 평가셋으로 lexical+dense 결합을 별도 비교한다.
3. 검색 후보와 직접 근거 선택을 실험 D에서 분리한다.
4. 근거 부족이면 답변 생성을 중단한다.

키워드를 보고 특정 법률을 직접 지정하는 규칙은 평가 질문을 외운 구현이 되기 쉽다. 후속 기본안은
lexical 순위와 dense 순위를 각각 보존하고 RRF로 결합하는 방식이다.
