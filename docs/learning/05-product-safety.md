...
운영 지원 범위는 날짜 상수가 아니다. 시작일은 한국 날짜의 오늘 이하인 수집·현재 parser·검색 가능
버전의 `effective_from` 전역 최솟값이고, 종료일은 한국 날짜의 오늘이며 양끝을 포함한다. 날짜를 생략한
요청도 한국 오늘을 사용한다. 이 계산은 법률별 과거 timeline의 gap·overlap을 모두 검사했다는 뜻은
아니다.

| 상태 | 뜻 | 사용자 행동 |
|---|---|---|
| `no_results` | 준비된 코퍼스에서 해당 경로·근거를 못 찾음 | 법령명·조문·사업 쟁점을 구체화 |
| `unsupported_corpus_date` | 준비된 검색 가능 코퍼스의 동적 지원 범위 밖 기준일 | 상태 API에 표시된 범위 안 날짜 선택 |
| `corpus_unready` | 전체 검색 게이트가 닫혔거나 오늘 유효 조문이 없거나 시간 identity를 완성하지 못해 검색을 닫음 | 잠시 뒤 재시도, 상태 확인 |
...
`/v1/corpus/status`는 동적으로 계산한 종료일과 준비 상태·사유를 항상 구분한다. 준비되지 않은 상태에서는
지원 시작일과 snapshot ID가 `null`일 수 있으므로 클라이언트가 임의 날짜나 이전 snapshot을 대신 표시하면
안 된다. 준비된 범위 밖 요청은 quota·임베딩·repository 검색보다 먼저 `422`, 준비되지 않은 상태는
`503`으로 반환한다.
