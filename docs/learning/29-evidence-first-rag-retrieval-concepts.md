...
후보 수를 넉넉히 잡는 목적은 정답을 놓치지 않는 것이다. 후보 10개를 모두 AI에 넣어야 한다는 뜻은 아니다. 현재 1,000문항 실험 D runner는 raw provision top 10을 승인 qrels와 비교하는 검색 평가 단계이며, 그중 직접 근거만 답변 문맥으로 고르는 단계는 후속 문맥·답변 평가로 분리한다.

## Article grouping

`Article grouping`은 같은 조에 속한 여러 항·호·목 청크를 하나의 조 후보로 묶는 것이다. 같은 제2조의 청크 여러 개가 순위를 독점하지 않게 한다.

production 조 점수는 해당 조의 청크 중 가장 높은 코사인 점수다. 작은 청크로 잘 찾되 답변 후보를 조로 정리하는 방식이다. 실험 D core 검색 지표는 부모·자식 qrel을 자동 합치지 않고 raw `provision_id`를 그대로 비교하므로 이 grouping을 사용하지 않는다.
...
`Evidence contract`는 고정 평가 질문에 어떤 공식 원문이 직접 근거이고 답에 어떤 요소가 필요한지 적은 검사 약속이다. 고정 필수 문구 하나만 찾는 방식은 표현이 조금만 달라져도 실패하고 넓은 질문의 여러 요건을 표현하지 못하므로 현재 gold 계약으로 사용하지 않는다.

```text
질문: 태양광 사업을 시작하려면 무엇을 준비해야 하는가?
필수 답변 요소: 허가, 계통연계, 검사
grade 2 qrels: 각 요소를 직접 뒷받침하는 조문 ID와 원문 SHA
grade 1 qrels: 직접 근거를 이해하는 데 필요한 보조 문맥
reference response: 답변·부분 답변·추가 질문·근거 부족 중 기대 동작과 기준 답
```

이 계약은 사용자가 질문을 승인한 뒤 검색 결과와 독립적으로 사람이 공식 원문을 검토해 만드는 평가용 정답표다. 정답 없는 질문은행은 이 주석 전의 임시 검토 단계이며, 임의의 실제 사용자 질문을 자동으로 이해하는 범용 판정기가 아니다.
...
2026-07-23의 작은 실험 C에서 corpus를 고친 뒤 범위 내 다섯 질문의 Law@1, Article Recall@3/5/10, Article MRR, Evidence Recall@3/5/10이 모두 1.0이었다. 이는 당시 질문과 corpus의 역사적 기준선일 뿐이며 새 1,000문항의 일반 성능이나 실행 결과가 아니다.

## Recall@K

`Recall@K`는 찾아야 할 정답 근거 전체 중 상위 K개에서 찾은 비율이다.

```text
한 질문의 grade 2 직접 qrel이 4개이고 top 3에서 3개를 찾음
질문별 Recall@3 = 3 / 4 = 0.75

grade 2 qrel을 하나라도 찾음
질문별 HitRate@3 = 1
```

정답 근거가 질문마다 하나뿐이면 Recall과 HitRate가 같은 값이 된다. 여러 근거가 필요한 질문에서는 하나만 찾아도 HitRate는 1이지만 Recall은 1보다 작을 수 있다. 첫 직접 근거의 순위 차이는 MRR@10으로 본다.
...
정확도를 높일 수 있지만 호출 시간과 비용이 늘고 별도 평가가 필요하다. 현재 실험 D runner는 AI reranker 없이 raw dense provision 순위를 승인 qrels와 비교한다.
