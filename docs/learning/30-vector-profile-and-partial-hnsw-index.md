...
외부 임베딩 API 호출과 DB 마이그레이션·적재를 한 작업으로 묶으면 중간 장애 때 어느 부분을 다시 해야 하는지 불명확해진다. 그래서 먼저 원문 없는 JSONL에 `provision_id + profile_key + source_text_sha256 + embedding`을 기록하고, corpus 전체와 해시가 맞는지 확인한 뒤 DB에 넣는다.

이 파일은 운영 원본이 아니라 재생성 가능한 파생 캐시다. 중단되어도 완료된 배치를 재사용하며, DB 적재 전에는 다음을 검증한다.

- 모든 현재 조각에 대응하는 벡터가 있는가
- 본문 입력 해시가 현재 corpus와 같은가
- 프로필과 차원이 같은가
- 모든 좌표가 유한하고 벡터 길이(norm)가 1인가
- DB가 해당 프로필을 지원하는 revision까지 적용됐는가
