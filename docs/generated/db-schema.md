...
> 기준 시점: 2026-07-18
> 생성 기준: `apps/api/migrations/versions/0001_legal_corpus.py` ~ `0005_conversation_history.py`
...
사용자 테이블은 `auth.users` 삭제를 기준으로 연쇄 삭제된다. 대화를 삭제하면 질문 턴과 해당 턴의 체크리스트 내보내기 메타데이터가 연쇄 삭제된다. 사용자 소유 테이블에는 RLS와 `auth.uid()` 소유권 정책을 적용했다. FastAPI의 pooler 직접 연결은 검증된 사용자 ID를 모든 소유 데이터 쿼리 조건에 사용한다.
