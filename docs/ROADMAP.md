...
| D-010 | Todo | [0057: 단일 단계 라우터와 라우터 불가 AI 응답](exec-plans/todo/0057-single-stage-router-and-failure-response.md) | 주 에이전트 | API 라우팅·응답·관측·테스트 | `legal_search`와 라우터 장애 AI 안내를 분리 | 라우팅 파이프라인 회귀, Ruff, 타입 검사 |

## 현재 TODO: D-010

- D-008은 독립 review 승인, PR #1 생성, GitHub Python/Web와 Vercel API/Web checks 통과로 완료됐다.
- D-010은 제안 단계다. 구현 전 `routing_unavailable`의 외부 route 명칭과 안내 문구를 확정한다.
