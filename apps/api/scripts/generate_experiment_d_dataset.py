...
_LIST_INTRODUCER = re.compile(r"다음\s+각\s+(?:호|목)")
...
def _is_evidence_eligible(item: SourceProvision) -> bool:
    return bool(
        item.article_path
        and _clean(item.content)
        and not _DELETED_PROVISION.match(item.content)
        and not _STRUCTURE_MARKER.match(item.content)
    )


def _is_eligible(item: SourceProvision) -> bool:
    return _is_evidence_eligible(item) and len(_clean(item.content)) >= 20


def _actor(item: SourceProvision) -> str | None:
    content = _clean(item.content)
    body = re.sub(
        r"^(?:제\d+조(?:의\d+)?(?:\([^)]*\))?|[①-⑳]|\d+(?:의\d+)?\.|[가-힣]\.)\s*",
        "",
        content,
    )
    boundary_candidates = [
    ...
    prefix = body[: min(boundary_candidates)] if boundary_candidates else body[:200]
    matches = re.findall(r"([^\s,.;:()]{2,40}?)(?:은|는|이|가)\s", prefix)
    rejected = {"경우", "때", "자", "것", "사항", "어느 하나", "다음", "필요"}
    role_suffixes = (
        "자",
        "장관",
        "기관",
    ...
    for candidate in reversed(matches):
        ...
        if actor in rejected:
            continue
        if not actor.endswith(role_suffixes):
            continue
        return actor
    if prefix.startswith("누구든지 "):
        return "누구든지"
    return None
...
def _semantic_question(item: SourceProvision, topic: str) -> tuple[str, str]:
    content = _clean(item.content)
    actor = _actor(item)
    subject = "누구에게나 " if actor == "누구든지" else f"{actor}에게 " if actor else ""
    if any(marker in content for marker in ("허가를 받아야", "인가를 받아야", "승인을 받아야")):
        question = f"{topic} 규정에서 {subject}요구되는 허가·인가·승인 요건은 무엇인가요?"
        template = "approval_requirement"
    elif any(marker in content for marker in ("신고하여야", "보고하여야")):
        question = f"{topic} 규정에서 {subject}요구되는 신고나 보고 의무는 무엇인가요?"
        template = "reporting_duty"
    elif any(marker in content for marker in ("해서는 아니", "하여서는 아니")):
        question = f"{topic} 규정에서 {subject}금지되는 행위는 무엇인가요?"
        template = "prohibition"
    elif "할 수 없다" in content:
        question = f"{topic} 규정에서 {subject}허용되지 않는 행위는 무엇인가요?"
        template = "not_permitted"
    elif "할 수 있다" in content:
        question = f"{topic} 규정에서 {subject}허용되는 조치나 행위는 무엇인가요?"
        template = "permitted_action"
    else:
        question = f"{topic} 규정에서 {subject}부과되는 의무는 무엇인가요?"
...
def _unique_semantic_candidates(
    candidates: list[SourceProvision],
...
def _evidence_items(
    ...
    *,
    evidence_scope: Literal["article", "leaf", "subtree"],
) -> list[SourceProvision]:
    if evidence_scope == "leaf" or item.article_path is None:
        return [item]
    article = by_article[(item.document_id, item.version_id, item.article_path)]
    if evidence_scope == "article":
        return article
    prefix = f"{item.path}/"
    return [
        candidate
        for candidate in article
        if candidate.path == item.path or candidate.path.startswith(prefix)
    ]


def _qrels(item: SourceProvision, relevant: list[SourceProvision]) -> list[dict[str, object]]:
    return [
        {
...
def _positive_case(
    *,
    ...
    index: int,
    template: str,
    evidence_scope: Literal["article", "leaf", "subtree"] = "article",
    ...
    relevant = _evidence_items(item, by_article, evidence_scope=evidence_scope)
    reference_items = relevant if evidence_scope == "subtree" else [item]
    return {
        "id": case_id,
        "split": _split(index),
        "category": category,
        ...
        "reference": _excerpt(" ".join(candidate.content for candidate in reference_items), 1600),
        "reference_contexts": [_excerpt(candidate.content, 1600) for candidate in relevant],
        "primary_evidence": {
...
def build_dataset(
    ...
    evidence_pool = [item for item in provisions if _is_evidence_eligible(item)]
    eligible = [item for item in evidence_pool if _is_eligible(item)]
    ...
    semantic_specs = {
        item.provision_id: _semantic_question(
            item,
            article_topics[(item.document_id, item.version_id, item.article_path)],
        )
        for item in semantic_candidates
    }
    semantic = _unique_semantic_candidates(semantic_candidates, semantic_specs)
    ...
    take(
        ...
        lambda item, case_id, index: _positive_case(
            ...
            evidence_scope="leaf" if "/" in item.path else "article",
        ),
    )
    take(
        "semantic_paraphrase",
        ...
        lambda item, case_id, index: _positive_case(
            ...
            question=semantic_specs[item.provision_id][0],
            item=item,
            by_article=by_article,
            index=index,
            template=semantic_specs[item.provision_id][1],
    ...
    def contrast(item: SourceProvision, case_id: str, index: int) -> dict[str, object]:
        ...
        question, template = semantic_specs[item.provision_id]
    ...
    corpus_fingerprint = hashlib.sha256(
        "\n".join(
            ...
            for item in sorted(evidence_pool, key=lambda candidate: candidate.provision_id)
        ).encode()
    ).hexdigest()
    return {
        "schema_version": 2,
        ...
        "corpus": {
            ...
            "evidence_provision_count": len(evidence_pool),
            "ambiguous_semantic_candidate_count": len(semantic_candidates) - len(semantic),
...
def validate_dataset(dataset: dict[str, object], provisions: list[SourceProvision]) -> None:
    ...
    descendants_by_id = {
        item.provision_id: [
            candidate
            for candidate in by_version[(item.document_id, item.version_id)]
            if candidate.path.startswith(f"{item.path}/")
        ]
        for item in provisions
    }
    ids: set[str] = set()
    ...
    for case in cases:
        ...
        if isinstance(primary, dict):
            primary_id = str(primary.get("provision_id"))
            source = by_id.get(primary_id)
            descendants = descendants_by_id.get(primary_id, [])
            if (
                source is not None
                and case.get("generation", {}).get("evidence_scope") == "subtree"
                and _LIST_INTRODUCER.search(source.content)
                and descendants
                and not any(item.provision_id in qrel_ids for item in descendants)
            ):
