...
def parse_history_json(body: str) -> list[HistoryVersion]:
    ...
    versions: dict[tuple[str, date | None], HistoryVersion] = {}
    for node in _walk(payload):
        ...
        versions[(version.mst, version.effective_from)] = version
    if not versions:
...
def parse_history_xml(body: str) -> list[HistoryVersion]:
    ...
    for node in root.iter():
        values = {
            child.tag.rsplit("}", 1)[-1]: _text(" ".join(child.itertext())) for child in node
        }
...
def effective_periods(versions: list[HistoryVersion]) -> list[EffectiveVersion]:
    """시행일 오름차순으로 ``[시행일, 다음 시행일)`` 효력 기간을 계산한다."""
    ...
    return [
        EffectiveVersion(
            ...
            effective_to=next_date[item.effective_from],
        )
        for item in ordered
    ]
